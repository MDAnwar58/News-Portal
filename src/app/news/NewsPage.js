...
export default function NewsPage() {
   ...
   useEffect(() => {
      const checkSearchParams = (limit, pageLoad) => {
         ...
         getNewsWithCategory(categorySlug, limit, pageLoad);
         getHighlights();
      };
   ...
   const getNewsWithCategory = async (slug, limit, pageLoad) => {
      if (pageLoad === false) {
      ...
      try {
         setSlug(slug);
         const res = await Axios.get(`/category/${slug}/news?limit=${limit}`);
   ...
   const onHandleLoadMore = useCallback(
      (nl) => {
         setLimit(limit + nl);
         getNewsWithCategory(slug, limit + nl, pageLoad);
      },
      [slug, limit, pageLoad]
   );
